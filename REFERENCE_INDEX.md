...
- Traffic cameras
  ...
  - Cache/scheduler: healthy signed URLs renew at five minutes and have a 15-minute hard maximum; unavailable cameras retry at 10 seconds. A Cloudflare request starts no more than four signing flows with three-way concurrency. The no-store API supports targeted, allowlisted recovery with `/api/cameras?refresh=1&cameraId=<id>` and returns sanitized 502/503 states during upstream or overlapping refresh failures.
