...
- Production artifact: `public/index.html` is served from the static-assets binding in `wrangler.jsonc`; `cloudflare/worker.js` implements `/api/wallboard`, `/api/cameras`, and the camera snapshot proxy in the same Worker. Provider modules are `cloudflare/analytics.js`, `cloudflare/providers.js`, `cloudflare/cameras.js`, `cloudflare/cameraImages.js`, `cloudflare/config.js`, and `cloudflare/cache.js`. Railway/another Node host and `WALLBOARD_API_ORIGIN` are not part of production.
...
- Traffic cameras
  - Config: `DRIVENC_API_KEY`; keep the numeric camera ID/label/priority lists aligned in `cloudflare/cameras.js` and `lib/trafficCameras.ts`.
  - IDs: `4208`, `4839`, `6120`, `5269`, `4210`, `4868`, `4876`, `4221` (I-26/US-25 corridor). DriveNC inventory is cached 90 seconds with in-flight/request de-duplication and stale metadata fallback.
  - Signed HLS implementation: `cloudflare/cameras.js` in production, `lib/trafficCameras.ts` plus `app/api/cameras/route.ts` locally. The flow gets the public DriveNC camera grant, exchanges its three opaque fields at Insight's secure-token endpoint, validates a single 64-hex token on the unchanged expected NCDOT manifest origin/path, and probes for `#EXTM3U`. The unsigned `Views[0].VideoUrl`, grant fields, failed URL, and `WWW-Authenticate` response are never serialized.
  - Cache/scheduler: healthy signed URLs renew at five minutes and have a 15-minute hard maximum; unavailable cameras retry at 10 seconds. Cloudflare persists timestamped inventory and signed entries in the Cache API, then rechecks source matching, signed-URL shape, and maximum age before hydrating a cold isolate. A Worker request starts no more than four signing flows with three-way concurrency. The no-store API supports targeted, allowlisted recovery with `/api/cameras?refresh=1&cameraId=<id>` and returns sanitized 502/503 states during upstream or overlapping refresh failures.
  - Client recovery: `public/index.html` and `app/wallboard/page.tsx` load snapshots first, preserve an unchanged live player, require `playing` within 18 seconds, sample `currentTime` every five seconds, and declare a 25-second visible-tab stall. A failed player is destroyed, the tile turns red over a cache-busted snapshot, and only that camera is force-checked 10 seconds later. Healthy players survive timeouts and partial metadata.
  - Snapshot fallback: the allowlisted local route `app/api/traffic-camera/[id]/route.ts` and Cloudflare route `/api/traffic-camera/<id>` (`cloudflare/cameraImages.js`) proxy DriveNC's public JPEG. They refresh upstream at most once per 60 seconds per camera, verify image content, de-duplicate concurrent requests, back off failures, and serve the last-good frame when available.
  - Layout remains an unlabeled `2 x 4` grid in `.ops-column`; labels are accessibility text, green dots identify playing HLS, amber identifies snapshot mode, red identifies recovery, and camera `4208` receives the amber priority frame.
...
- If DriveNC changes either public signing endpoint or its HLS URL shape, update the strict validators and `npm run test:cameras` before changing client behavior. Never relax validation merely to make a new URL play.
